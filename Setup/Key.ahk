
#Include %A_WorkingDir%\Config\All.ahk
#Include %A_WorkingDir%\Config\Software.ahk

#Include %A_WorkingDir%\Lib\Json.ahk
#Include %A_WorkingDir%\Lib\TrayIcon.ahk

#Include %A_WorkingDir%\Tool\Base.ahk
#Include %A_WorkingDir%\Tool\Change.ahk
#Include %A_WorkingDir%\Tool\Help.ahk
#Include %A_WorkingDir%\Tool\File.ahk
#Include %A_WorkingDir%\Tool\Init.ahk
#Include %A_WorkingDir%\Tool\Global.ahk
#Include %A_WorkingDir%\Tool\Language.ahk
#Include %A_WorkingDir%\Tool\Mouse.ahk
#Include %A_WorkingDir%\Tool\Other.ahk
#Include %A_WorkingDir%\Tool\Window.ahk
#Include %A_WorkingDir%\Tool\Control.ahk
#Include %A_WorkingDir%\Tool\KeyMap.ahk
#Include %A_WorkingDir%\Tool\Office.ahk
#Include %A_WorkingDir%\Tool\Keyboard.ahk

#Include %A_WorkingDir%\Key\All.ahk
#Include %A_WorkingDir%\Key\LWin.ahk
#Include %A_WorkingDir%\Key\RWin.ahk
#Include %A_WorkingDir%\Key\Ctrl.ahk
#Include %A_WorkingDir%\Key\Alt.ahk
#Include %A_WorkingDir%\Key\Shift.ahk
#Include %A_WorkingDir%\Key\Space.ahk
#Include %A_WorkingDir%\Key\CapsLock.ahk


#SingleInstance Force
#NoTrayIcon
